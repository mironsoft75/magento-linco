...
class Factory {
    ...
    public function createLayeredCategoryFilter($name, $ids, $operation = Operation::LOGICAL_OR) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Filters\LayeredFilters\CategoryFilter',
    ...
    public function createLayeredDecimalFilter($name, $attributeId, $ranges, $isToRangeInclusive = false, $operation = Operation::LOGICAL_OR) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Filters\LayeredFilters\DecimalFilter',
    ...
    public function createLayeredMinMaxFilter($name, $attributeId, $maxAttributeId, $ranges, $isToRangeInclusive = false, $operation = Operation::LOGICAL_OR) {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationSliders\Filters\MinMaxFilter',
    ...
    public function createLayeredDropdownFilter($name, $attributeId, $optionIds, $operation = Operation::LOGICAL_OR) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Filters\LayeredFilters\DropdownFilter',
            compact('name', 'attributeId', 'optionIds', 'operation')
        );
    }

    public function createLayeredPriceFilter(
        $name,
        $attributeId,
        $ranges,
        $inclusive = false,
    ...
    {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Filters\LayeredFilters\PriceFilter',
    ...
    public function createOptimizedDropdownFacet(
        $name,
        $attributeId,
        $selectedOptionIds,
    ...
    {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Dropdown\OptimizedFacet',
            compact(
                'name',
                'attributeId',
                'selectedOptionIds',
    ...
    public function createFacetBatch($type) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Contracts\FacetBatch',
            compact('type')
        );
    }

    public function createStandardDropdownFacet(
        $name,
        $attributeId,
        $selectedOptionIds,
        $hideWithSingleVisibleItem,
    ...
    {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Dropdown\StandardFacet',
            compact(
                'name',
                'attributeId',
                'selectedOptionIds',
    ...
    public function createOptimizedSwatchFacet(
        $name,
        $attributeId,
        $selectedOptionIds,
        $hideWithSingleVisibleItem,
    ...
    {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Swatch\OptimizedFacet',
            compact(
                'name',
                'attributeId',
                'selectedOptionIds',
    ...
    public function createStandardSwatchFacet(
        $name,
        $attributeId,
        $selectedOptionIds,
        $hideWithSingleVisibleItem,
    ...
    {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Swatch\StandardFacet',
            compact(
                'name',
                'attributeId',
                'selectedOptionIds',
    ...
    public function createChildCategoryFacet($name, $appliedCategory, $hideWithSingleVisibleItem) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Category\ChildFacet',
    ...
    public function createEqualizedRangePriceFacet($name, $appliedRanges, $hideWithSingleVisibleItem) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Price\EqualizedRangeFacet',
    ...
    public function createEqualizedCountPriceFacet($name, $appliedRanges, $hideWithSingleVisibleItem) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Price\EqualizedCountFacet',
            compact('name', 'appliedRanges', 'hideWithSingleVisibleItem')
        );
    }
    ...
    public function createManualRangePriceFacet($name, $appliedRanges, $hideWithSingleVisibleItem) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Price\ManualRangeFacet',
            compact('name', 'appliedRanges', 'hideWithSingleVisibleItem')
        );
    }
    ...
    public function createEqualizedRangeDecimalFacet($name, $attributeId, $appliedRanges, $hideWithSingleVisibleItem) {
        return $this->objectManager->create(
            'Manadev\ProductCollection\Facets\Decimal\EqualizedRangeFacet',
    ...
    public function createSliderMinMaxDecimalFacet($name, $attributeId, $appliedRanges, $calculateSliderMinMax, $numberFormat, $showThousandSeparator, $minMaxRole, $minAttributeCode, $precision, $maxFilterId = null){
        return $this->objectManager->create(
            'Manadev\LayeredNavigationSliders\Facets\Decimal\MinMaxSliderRangeFacet',
    ...
    public function createSliderRangeDecimalFacet($name, $attributeId, $appliedRanges, $calculateSliderMinMax, $numberFormat, $showThousandSeparator, $precision) {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationSliders\Facets\Decimal\SliderRangeFacet',
    ...
    public function createSliderRangePriceFacet($name, $attributeId, $appliedRanges, $calculateSliderMinMax, $numberFormat, $showThousandSeparator, $precision) {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationSliders\Facets\Price\PriceSliderRangeFacet',
            compact('name', 'attributeId', 'appliedRanges', 'calculateSliderMinMax', 'numberFormat', 'showThousandSeparator', 'precision')
        );
    }

    public function createSliderRangeDropdownFacet(
        $name,
        $attributeId,
        $selectedOptionIds,
        $calculateSliderMinMax,
        $showSelectedOptionsFirst,
        $sortBy
    )
    {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationSliders\Facets\Dropdown\DropdownSliderRangeFacet',
            compact(
                'name',
                'attributeId',
                'selectedOptionIds',
    ...
    public function createChildAllFacet($name, $appliedCategory, $currentCategory, $hideEmptyProductCount) {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationCategoryTree\Facets\Category\ChildAllFacet',
    ...
    public function createChild2ndLevelFacet($name, $appliedCategory, $currentCategory, $hideEmptyProductCount) {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationCategoryTree\Facets\Category\Child2ndLevelFacet',
            compact('name', 'appliedCategory', 'currentCategory', 'hideEmptyProductCount')
        );
    }
    ...
    public function createParentChildrenFacet($name, $appliedCategory, $currentCategory, $hideEmptyProductCount) {
        return $this->objectManager->create(
            'Manadev\LayeredNavigationCategoryTree\Facets\Category\ParentChildrenFacet',
            compact('name', 'appliedCategory', 'currentCategory', 'hideEmptyProductCount')
        );
    }
