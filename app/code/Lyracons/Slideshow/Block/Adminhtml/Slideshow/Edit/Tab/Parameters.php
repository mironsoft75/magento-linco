<?php
namespace Lyracons\Slideshow\Block\Adminhtml\Slideshow\Edit\Tab;
...
class Parameters extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
     ...
     * @param \Lyracons\Slideshow\Model\Config\Source\ListEffect $listEffect        
     * @param array $data
     */
    public function __construct(
    \Magento\Backend\Block\Template\Context $context, \Magento\Framework\Registry $registry, \Magento\Framework\Data\FormFactory $formFactory, \Lyracons\Slideshow\Model\Config\Source\ListEffect $listEffect, \Lyracons\Slideshow\Model\Config\Source\Truefalse $boolean, array $data = []
    )
    {
    ...
    protected function _prepareForm()
    {
        ...
        $model = $this->_coreRegistry->registry('lyracons_slideshow');


        $fieldset = $form->addFieldset(
            'settings_fieldset', ['legend' => __('Slideshow Options'), 'class' => 'fieldset-wide']
        );
        ...
        $this->setForm($form);
        $this->setChild(
            'form_after', $this->getLayout()->createBlock(
                    'Magento\Backend\Block\Widget\Form\Element\Dependence'
                )->addFieldMap(
                    "{$htmlIdPrefix}controlNavThumbs", 'controlNavThumbs'
                )
                ->addFieldMap(
                    "{$htmlIdPrefix}thumbWidth", 'thumbWidth'
                )
                ->addFieldMap(
                    "{$htmlIdPrefix}thumbHeight", 'thumbHeight'
                )
                ->addFieldDependence(
                    'thumbWidth', 'controlNavThumbs', '1'
                )
                ->addFieldDependence(
                    'thumbHeight', 'controlNavThumbs', '1'
                )
        );
