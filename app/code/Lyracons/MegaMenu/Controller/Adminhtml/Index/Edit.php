...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Lyracons_MegaMenu::edit');
    }

    protected function _initAction()
    {
        $resultPage = $this->resultPageFactory->create();
        $resultPage->setActiveMenu('Lyracons_MegaMenu::megamenu')
    ...
    public function execute()
    {
        ...
        $model = $this->_objectManager->create('Lyracons\MegaMenu\Model\Megamenu');
        $model->setMenuId($id);
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
