		...
		return this.each(function(id,el){
			...
			$.fn.extend({
				...
				removeMenuItem: function(){
					return this.each(function(){
						...
						lcmenu.confirm(conf.phrase.warning,conf.phrase.removeItemWarning,{confirm: removeItem});
					});
				},
				...
				eventOnClickAddLink: function(){
					return this.each(function(){
						...
						$this.updateNoChildrenClass();
						$('.content-row',$cloneItem).lcAccordion();
					});
				},
			...
			function menuItemTypeTmpl(){
				...
				$('[data-type="category"]',$menuTypeList).each(function(){
					var $field = $(this);
					lcInstantiateChooser($field.attr('id'));
				});
				...
				$(conf.items,$menuTypeList).draggable({
					...
					stop: function(event, ui) {
						...
						$('.content-row',ui.helper).lcAccordion();
					}
				});
			};
			...
			function menuActions(){
				...
				$previewBtn.on('click',function(){
					if(previewUrl != null){
						var menuJson = lcmenu.menuItemToJson();
							...
							opened: function () {
								...
								$.ajax({
									...
									success: function(res){
										...
										('.lc-menu',$previewArea).first().trigger('contentUpdated')
			...
			function attachIconToItemHeading(textEl,value, type){
				...
				if(type == 0){
					...
					if($textEl.data('name') == 'icon_img'){
						$previewIcon.html('<a class="preview-image" onclick="lcmenu.viewfull(this)" href="javascript:void(0)" data-href="'+value+'"><img src="'+value+'" /></a>');
					}
			...
			$.fn.lcAccordion = function(options){
				/*var defaultConfig = {
				...
				return this.each(function(){
					...
					$(apiConfig.header,$this).each(function(){
						var $title = $(this);
						if(typeof $title.data('lcAccordion') == 'undefined'){
							...
							$title.data('lcAccordion',true);	
						}
					});					
				});*/
			}
			$('.content-row').lcAccordion();
			...
			eval('window.'+conf.windowId+' = lcmenu');
		});
	};
})(jQuery);