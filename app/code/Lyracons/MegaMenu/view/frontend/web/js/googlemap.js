	...
	$.widget('lyracons.googlemap', {
		...
		_create: function(){
			...
			require([config.jsSource],function(){
				...
				if(self.element.parents('.lc-menu').length > 0){
					var $menu = self.element.parents('.lc-menu').first(),
					$li = self.element.parents('li.level0').first(),
					$ul = $li.find('> .groupmenu-drop');
					if(self.element.parents('.lc-slide').length || self.element.parents('.lc-fade').length || self.element.parents('.lc-normal').length){
